...
def base_create():
    ...
    curs.execute(f"""INSERT INTO {cfg.tabl_file}(device_id, device_path, device_reg, date_in, date_out, fio, tabnum, department)
...
def test_true_base_check_flask_name():
    fiotab = 'four'
    assert sql.base_check_flask_name(fiotab)==[('4', 4, 4, 4, 'four', 'four', 4, 'four')]

def test_true_base_check_flask_tabnum():
    fiotab = 'five'
    assert sql.base_check_flask_name(fiotab)==[('5', 5, 5, 5, 'five', 5, 'five', 'five')]